...
els.loadDemoData.addEventListener("click", () => {
  state = createDemoState();
  currentView = "admin";
  saveAndRender("Demo cargada. Ya podes seleccionar estantes, cargar productos o entrar como operario.");
});
...
els.warehouseForm.addEventListener("submit", (event) => {
  ...
  saveAndRender();
  showStatus("Almacen guardado. Ahora podes crear estantes.");
});
...
els.shelfForm.addEventListener("submit", (event) => {
  ...
  if (!state.warehouse) {
    showStatus("Primero guarda los datos del almacen para poder crear estantes.", "error");
    return;
  }
  ...
  if (exists) {
    showStatus("Ese codigo de estante ya existe. Usa otro codigo.", "error");
    return;
  }
  ...
  els.shelfForm.reset();
  state.selected = { shelfCode: code, sectionId: null };
  saveAndRender(`Estante ${code} creado. Seleccionalo y elegi una celda para cargar productos.`);
});
...
els.stockForm.addEventListener("submit", async (event) => {
  ...
  els.stockForm.reset();
  saveAndRender(`Producto ${code} guardado en la ubicacion seleccionada.`);
});
...
els.movementForm.addEventListener("submit", (event) => {
  ...
  saveAndRender("Movimiento registrado correctamente.");
});
...
function saveAndRender(message = "") {
  ...
  render();
  if (!saved) {
    showStatus("No se pudo guardar en el navegador. Proba reiniciar la demo o usar fotos mas livianas.", "error");
  } else if (message) {
    showStatus(message);
  }
}
  ...
  const shelves = [
    {
      ...
      rows: 3,
      position: { x: 7, y: 10 },
      sections: createShelfSections("A-01", 3, 4)
    },
    {
      code: "B-02",
      width: 3,
      ...
      rows: 3,
      position: { x: 44, y: 16 },
      sections: createShelfSections("B-02", 3, 3)
    }
  ];

  shelves[0].sections[0].items.push({ code: "SKU-1001", name: "Tornillos 8 mm", qty: 120, photo: "" });
  shelves[0].sections[5].items.push({ code: "SKU-2040", name: "Cajas de rulemanes", qty: 38, photo: "" });
  shelves[1].sections[2].items.push({ code: "SKU-3302", name: "Filtros industriales", qty: 24, photo: "" });

  return {
    warehouse: {
      name: "Deposito Planta AGD",
      area: 120,
      width: 12,
      length: 10
    },
    shelves,
    reports: [],
    selected: { shelfCode: "A-01", sectionId: "A-01-F1-C1" }
  };
}
